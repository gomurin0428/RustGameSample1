...
impl GameState {
    ...
    /// Constructs a new GameState from a prepared GameBootstrap.
    ///
    /// Initializes simulation clock, RNG, countries, commodity market, scripted event engine,
    /// industry engine, and systems facade, then captures the initial fiscal history.
    ///
    /// # Examples
    ///
    /// ```no_run
    /// // prepare a GameBootstrap with the desired initial state
    /// let bootstrap = /* build GameBootstrap */ unimplemented!();
    /// let game = realgeopolitics::game::state::GameState::new(bootstrap);
    /// ```
    ...
    /// Retrieves the fiscal snapshot for the country at the specified zero-based index.
    ///
    ...
    ///
    /// ```
    /// # use realgeopolitics::game::state::GameState;
    /// # fn example(game: &GameState) {
    /// let snapshot = game.fiscal_snapshot_of(0).expect("country index must exist");
    /// // use `snapshot`...
    /// # }
    /// ```
    ...
    /// Locate the index of a scripted event by its identifier.
    ///
    /// # Parameters
    ///
    /// - `id`: The scripted event identifier to look up.
    ///
    /// # Returns
    ///
    /// `Some(index)` containing the event's index if found, `None` otherwise.
    ///
    /// # Examples
    ///
    /// ```ignore
    ...
    /// if let Some(i) = idx {
    ///     // found event at index `i`
    ///     assert!(i < state.scripted_events.len());
    /// } else {
    ///     // no event with that id
    ///     assert!(state.scripted_events.find_index("resource_boom").is_none());
    /// }
    /// ```
    ...
    /// Returns the description text for the scripted event with the given id, if present.
    ///
    /// # Examples
    ///
    /// ```ignore
    ...
    /// Executes a scripted event template at the current simulation time and returns the textual reports produced by that execution.
    ///
    /// `template_idx` is the index of the scripted event template to execute; the engine will run the template against the current country states using the simulation clock's minutes.
    ///
    /// # Returns
    ///
    /// `Vec<String>` containing the textual reports generated by the scripted event.
    ///
    /// # Examples
    ///
    /// ```no_run
    /// // Assuming `game` is a mutable GameState already initialized:
    /// let mut game = /* obtain GameState */;
    ...
    /// Record a fiscal snapshot for every country at the simulation's current time.
    ///
    /// This updates each country's fiscal history by pushing a snapshot using the
    /// GameState's current simulation minutes.
    ///
    /// # Examples
    ///
    /// ```no_run
    /// // Given a mutable `GameState` instance, capture the current fiscal history:
    /// let mut game = /* construct or obtain a GameState */ ;
    /// game.capture_fiscal_history();
    /// ```
